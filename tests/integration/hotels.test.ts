...
describe("GET /hotels/:id", () => {
    ...
    it ("should respond with status 404 when at least one hotel is listed but hotelId doesn't exists ", async () => {


        const response = await server.get(`/hotels/${hotelId +100}`).set("Authorization", `Bearer ${token}`)
